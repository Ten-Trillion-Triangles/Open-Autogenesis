...
//
// User-driven verification of the snapshot resume flow. Two phases.
//
// PHASE 1: fresh login, PLAY single player, submit ONE turn, wait for the
// opponent's turn to be active (Active actor != me, work stream glow,
// timer reset), verify music is audible via Web Audio API hook, close.
//
// PHASE 2: a SECOND probe invocation that starts FRESH after the user
// kills and restarts all dev servers. Login again, expect ResumeOrNewDialog,
// click Resume, verify back in game, NOT your turn, AI work stream moving,
// music playing. Take a PNG screenshot for proof.
//
// NOTE: This probe is split across two runs because the user must
// manually kill + restart the servers between phases. Run phase 1 first,
// then in another shell:
//     pkill -f "server:run"; pkill -f "server-extend:run"; pkill -f runKvisionNoHotReload; sleep 5
//     ./debugger/scripts/start_servers.sh
// then run again with --phase=2 (after server restart).
//
// Output: PNG screenshot at artifacts-echo-verify/screenshot.png when
// all checks pass.
...
async function enterSinglePlayerGame(page) {
    ...
    await dumpDomSnapshot(page, 'commander-selection')
    ...
    await dumpDomSnapshot(page, 'after-play-click')
    ...
    if (await page.locator('[data-testid="gameplay-ui"]').count() === 0) {
        await dumpDomSnapshot(page, 'no-gameplay')
...
async function captureGameState(page, label) {
    const state = await page.evaluate(() => {
        ...
        // Use the body text matches more precisely by looking for the big button text
        ...
        // Detect music via Web Audio API hooks (set by addInitScript).
...
async function playFirstTurnAndWaitForOpponent(page) {
    ...
    let logOffset = 0
    // Track server log file size to detect new lines (ESM-friendly dynamic import)
    const { statSync: stat, openSync: openF, readSync: readF, closeSync: closeF } = await import('fs')
    stat('/tmp/autogenesis-proxy/srv.log').size
    // Derive myCommanderBase from the myName (which is "<commander>Main", e.g. "AUongfa834nfaMain")
    const myCommanderBase = myName.endsWith('Main') ? myName.slice(0, -4) : myName
    log(`  my commander base = ${myCommanderBase}`)
    for (let i = 0; i < 720; i++) {  // up to 12 minutes for full turn + opponent start
        // Check server log for "<other-name>-<round>-<oppIdx>" as processed (opponent's turn)
        if (!turnIndexAdvanced) {
            try {
                ...
                if (currentLogSize > logOffset) {
                    ...
                    // Look for "as processed" lines that are NOT my commander turn
                    // My turn key: "AUongfa834nfa-1-0" — opponent's: "John Kirby-1-1" or similar.
        ...
        // The DOM `actor` field shows the active actor. After turn index advances,
        // the server starts AI generation for the next actor.
        // We need BOTH the actor != myName AND the turnIndexAdvanced log signal.
        if (state.activeActor && myName && state.activeActor !== myName && turnIndexAdvanced) {
            oppTurnFound = true
            log(`  opponent turn detected at iter ${i} (${(Date.now() - startWait) / 1000}s elapsed): actor=${state.activeActor}, turnIndexAdvanced=true`)
            break
        }
    ...
    if (stateBeforeClose.audioCount === 0) {
        ...
        log('  inspecting AudioContext state and any media elements')
        const audioInfo = await page.evaluate(() => ({
            ctxState: window.__audioContext && window.__audioContext.state,
            audioEls: Array.from(document.querySelectorAll('audio')).length,
            ctxExists: !!window.__audioContext,
            bufferSourceCount: window.__audioActiveBufferSources || 0,
        }))
        log(`  audioInfo: ${JSON.stringify(audioInfo)}`)
    }
...
async function runPhase1() {
    ...
    log(`  opponentTurn=${phase1End.opponentTurn}`)
    ...
    await writeFile('/tmp/echo-phase1.json', JSON.stringify({
        ...
        opponentTurn: phase1End.opponentTurn,
...
async function runPhase2() {
    ...
    for (let i = 0; i < 60; i++) {
        const result = await page2.evaluate(() => {
            ...
            if (bodyText.includes('Saved game found') || bodyText.includes('resume your saved') || bodyText.includes('Resume Saved')) {
                return { found: 'text', text: bodyText.slice(0, 300) }
            }
            return null
        })
        if (result) {
    ...
    log('  Step P2.3: dismiss any post-resume messageBox, wait for GameplayUI')
    for (let i = 0; i < 60; i++) {
    ...
    log('  Step P2.4: wait 25s for chunked ui.loadMapPack + ui.updateWorld to fully populate')
    await page2.waitForTimeout(25_000)

    log('  Step P2.5: capture state after resume (verify: gameplay mounted, NOT your turn, work stream moving, music playing)')
    ...
    // Click OK on the Match Resumed modal if it's showing
    ...
    await page2.waitForTimeout(3000) // let game UI render after dismiss

    // Take the actual game UI screenshot AFTER clicking OK
    ...
    await writeFile('/tmp/echo-phase2.json', JSON.stringify({
        ...
        opponentTurn: phase2State.opponentTurn,
        activeActor: phase2State.activeActor,
        workStreamGlow: phase2State.workStreamGlow,
        audioCount: phase2State.audioCount,
    }, null, 2))

    // Keep the browser open a few seconds to let music + work stream move visibly
    log('  Step P2.7: wait 5s with browser open to confirm work stream moves + audio continues')
    ...
    // Exit code reflects all required checks
    const ok = phase2State.round != null
        && !phase2State.yourTurn
