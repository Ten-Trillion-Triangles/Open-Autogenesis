...
/**
 * JVM implementation of the gRPC RPC bridge.
 * Manages client connections and provides RPC communication for server environments.
 */
actual object GrpcRpcBridge
{
    private const val playerIdPrefix = "grpc-client"
    ...
    actual suspend fun connect(playerId: String, endpoint: String)
    {
        ...
        {
            Logger.info(LogCategory.NETWORK, "GrpcRpcBridge: already connected as $playerId")
            return
        }
        ...
        {
            grpcClient.connect()
            Logger.info(LogCategory.NETWORK, "GrpcRpcBridge: connect() invoked for $playerId@$endpoint")
        }
        catch(err: Throwable)
        {
            Logger.error(LogCategory.NETWORK, "Failed to start gRPC client: ${err.message}")
            throw err
        }
     ...
     *
     * @return A randomly generated player ID string with "grpc-client" prefix
     */
