...
const distDir = join(rootDir, 'audioTracksEditor', 'build', 'dist', 'js', 'productionExecutable')
const webpackDir = join(rootDir, 'audioTracksEditor', 'build', 'kotlin-webpack', 'js', 'productionExecutable')
...
async function main()
{
    ...
    console.log(`audio-tracks-editor-e2e: static server listening at http://${host}:${port}`)
...
function runGradleBuild()
{
    return new Promise((resolve, reject) => {
        console.log('audio-tracks-editor-e2e: building production bundle via :audioTracksEditor:jsBrowserDistribution')
        const child = spawn(gradleCommand(), [':audioTracksEditor:jsBrowserDistribution', '--no-daemon'], {
            cwd: rootDir,
            stdio: 'inherit'
        })
        gradleProcess = child
